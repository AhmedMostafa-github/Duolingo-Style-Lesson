// Fallback store without persistence for when AsyncStorage is not available
import { create } from 'zustand';
import { devtools } from 'zustand/middleware';
import { LessonStore, initialState } from './lessonStore';

// Create a non-persistent version of the store
  ...
  return create<LessonStore>()(
    devtools(
      (set, get) => ({
        ...
        loadLesson: async () => {
          console.log('🔄 loadLesson called (fallback mode)');
          ...
          try {
            ...
            console.log('🎯 Lesson loaded (fallback mode):', {
        ...
        // Start a new lesson
        startLesson: () => {
          console.log('🔄 startLesson called (fallback mode)');
          const { lesson } = get();
          if (!lesson) return;
          ...
          console.log('🚀 Lesson started (fallback mode):', {
            title: lesson.title,
        ...
        resetLesson: () => {
          ...
          console.log('Lesson reset (fallback mode)');
        },
        ...
        submitAnswer: (exerciseId: string, payload: any) => {
          ...
          console.log(
            `📝 Answer submitted for ${exerciseId} (fallback mode):`,
            {
              type: payload.type,
              isCorrect,
              streak: newStreak,
        ...
        decrementHeart: () => {
          ...
          if (hearts > 0) {
            set({ hearts: hearts - 1 });
            console.log(
              `💔 Heart lost. Remaining hearts: ${hearts - 1} (fallback mode)`,
            );
          }
        },
        ...
        complete: () => {
          ...
          console.log('🎉 Lesson completed! (fallback mode)', {
            lesson: lesson.title,
            totalXp: xp,
        ...
        setLocale: (locale: string) => {
          set({ locale });
          console.log('🌍 Locale changed to:', locale, '(fallback mode)');
        },

        // Set theme
        setTheme: (theme: 'light' | 'dark') => {
          set({ theme });
          console.log('🎨 Theme changed to:', theme, '(fallback mode)');
        },
        ...
        resetLessonCompletely: () => {
          ...
          console.log('🔄 Lesson completely reset (fallback mode)');
        },

        // Set navigation state
        setShouldNavigateToPlayer: (should: boolean) => {
          set({ shouldNavigateToPlayer: should });
          console.log('🧭 Navigation state set (fallback mode):', should);
        },
      }),
      {
        name: 'lesson-store-fallback',
      },
    ),
  );
};
